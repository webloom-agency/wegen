...
export function ToolSelector({
  ...
  const [appStoreMutate, allowedAppDefaultToolkit, allowedMcpServers] =
    appStore(
      useShallow((state) => [
        state.mutate,
            ...
            selectedMcpServerList.map((server) => (
              <DropdownMenuSub key={server.id}>
                ...
                >
                  ...
                  <span
                    className={cn("truncate", !server.checked && "opacity-40")}
                  >
                    {server.serverName}
                  </span>
                  {Boolean(server.error) ? (
                    <span
                      className={cn(
                        "text-xs text-destructive ml-1 p-1 rounded",
                      )}
                    >
                      error
                    </span>
