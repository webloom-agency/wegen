...
function resolvePlaceholdersInValue(
  value: any,
  ...
  if (typeof value === "string") {
    ...
    // Extract tokens of the form /{ ...valid JSON... } with balanced braces
    const tokens: { token: string; key?: OutputSchemaSourceKey; resolved?: any }[] = [];
        ...
        while (j < text.length) {
          ...
          if (ch === "}") {
            ...
            if (depth === 0) {
              ...
              try {
                const jsonPart = text.slice(i + 1, j + 1); // remove leading '/'
                const key = JSON.parse(jsonPart) as OutputSchemaSourceKey;
                tokens.push({ token, key });
              } catch (_e) {
                // ignore invalid JSON placeholder
              }
    ...
    for (const t of tokens) {
      if (!t.key) continue;
      let resolved = getOutput(t.key);
      if (resolved === undefined || resolved === null) resolved = "";
      t.resolved = resolved;
    }

    // If the entire string is exactly one placeholder, return the resolved value directly
    if (tokens.length === 1 && text.trim() === tokens[0].token) {
      return tokens[0].resolved;
    }

    // Otherwise, replace placeholders inside the string with their string representations
    for (const t of tokens) {
      const strVal =
        typeof t.resolved === "string" || typeof t.resolved === "number"
          ? String(t.resolved)
          : t.resolved
            ? JSON.stringify(t.resolved)
            : "";
      replaced = replaced.split(t.token).join(strVal);
    }
    return replaced;
  }
  if (Array.isArray(value)) {
    return value.map((v) => resolvePlaceholdersInValue(v, getOutput));
  }
  if (value && typeof value === "object") {
    const out: Record<string, any> = {};
    for (const [k, v] of Object.entries(value)) {
      out[k] = resolvePlaceholdersInValue(v, getOutput);
    }
    return out;
  }
  return value;
